...
public class MainActivity extends AppCompatActivity {
    ...
    public boolean drg_flag = true;// true为角度，false为弧度，默认角度
    public double pi = 4 * Math.atan(1);// π值
    public boolean tip_lock = true;// true为正确，可以继续输入，false错误，输入锁定，默认可以继续输入
    public boolean equals_flag = true;// 是否在按下=之后输入，true为之前，false为之后，默认按下等于号之前输入
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);
        InitWigdet();//初始化所有组件
        AllWigdetListener();//设置监听器
    }
...
// 为+-x÷等按键绑定监听器
        ...
        drg.setOnClickListener(actionPerformed);//角度弧度转换
        mc.setOnClickListener(actionPerformed);
        c.setOnClickListener(actionPerformed);
    }
    //键盘命令捕捉
    String[] TipCommand = new String[500];//键盘命令字符串
...
// 检测输入是否合法
            if (equals_flag == false//按下=号之后输入，输入不合法
                    && "0123456789.()sincostanlnlogn!+-×÷√^".indexOf(command) != -1) {//此字符串中可以检索到command，返回索引
// 检测显示器上的字符串是否合法
                if (right(str)) {
                    if ("+-×÷√^)".indexOf(command) != -1) {
                        for (int i = 0; i < str.length(); i++) {
                            TipCommand[tip_i] = String.valueOf(str.charAt(i));//把变量转换为字符串赋给之，检索str中第一个字符
                            tip_i++;
                        }
                        vbegin = false;//接着输入
                    }
...
// 若果点击了“DRG”，则切换当前弧度角度制，并将切换后的结果显示到按键上方。
            } else if (command.compareTo("DRG") == 0 && tip_lock) {//若点击了DRG且输入正确
                if (drg_flag == true) {//如果默认是角度
                    drg_flag = false;//转换为弧度
                    _drg.setText(" RAD");//显示弧度
                } else {//若为弧度
                    drg_flag = true;//转换为角度
                    _drg.setText(" DEG");//显示角度
                }
// 如果输入时退格键，并且是在按=之前
            } else if (command.compareTo("Bksp") == 0 && equals_flag) {//equals_flag默认为按等于号之前输入
// 一次删除3个字符
                if (TTO(str) == 3) {
                    if (str.length() > 3)
...
// 若之前输入的字符串不合法则删除全部字符
                ...
                if (tip_i > 0)
                    tip_i--;
            } else if (command.compareTo("Bksp") == 0 && equals_flag == false) {// 如果是在按=之后输入退格键
// 将显示器内容设置为0
                input.setText("0");
                vbegin = true;//重新输入
                tip_i = 0;
...
// 表示命令类型
        ...
        int bracket = 0;// 括号数
        if (tipcommand1.compareTo("#") == 0// “+-x÷√^”不能作为第一位
...
// 检测小数点的重复性，Tipconde1=0,表明满足前面的规则
        if (Tipcode1 == 0 && tipcommand2.compareTo(".") == 0) {
            int tip_point = 0;
            for (int i = 0; i < tip_i; i++) {//循环检测小数点
// 若之前出现一个小数点点，则小数点计数加1
                if (TipCommand[i].compareTo(".") == 0) {//检测到小数点计数加1
                    tip_point++;
                }
                if (TipCommand[i].compareTo("sin") == 0// 若出现以下几个运算符之一，小数点计数清零，说明输完一个数字了
            ...
            tip_point++;
            if (tip_point > 1) {// 若小数点计数大于1，表明小数点重复了，说明在一个数字中有两个小数点
                Tipcode1 = 8;
            }
        }
        if (Tipcode1 == 0 && tipcommand2.compareTo(")") == 0) {// 检测右括号是否匹配
            int tip_right_bracket = 0;
            for (int i = 0; i < tip_i; i++) {//从开始循环
                if (TipCommand[i].compareTo("(") == 0) {// 如果出现一个左括号，则计数加1
                    tip_right_bracket++;
                }
// 如果出现一个右括号，则计数减1
...
// 检查输入=的合法性
        if (Tipcode1 == 0 && tipcommand2.compareTo("=") == 0) {//没有输入错误
// 括号匹配数
            int tip_bracket = 0;
...
// 清屏后输出
    ...
    */
    private int TTO(String str) {//删除时分类型删除，判断删几个字符
        if ((str.charAt(str.length() - 1) == 'n'
                ...
                .charAt(str.length() - 3) == 'c')//cos
                || (str.charAt(str.length() - 1) == 'n'
                && str.charAt(str.length() - 2) == 'a' && str
                .charAt(str.length() - 3) == 't')//tan
                || (str.charAt(str.length() - 1) == 'g'
                && str.charAt(str.length() - 2) == 'o' && str
                .charAt(str.length() - 3) == 'l')) {//log
            return 3;
    ...
    */
    ...
    /*整个计算核心，只要将表达式的整个字符串传入calc().process()就可以实行计算了 算法包括以下几部分：
    * 1、计算部分
    ...
    */
    public class calc {
        ...
        * 扫描完后对剩下的运算符与数字依次计算
        */
        public void process(String str) {//计算部分
...
// sin
                                case 's':
                                    if (drg_flag == true) {//默认角度
                                        number[topNum - 1] = Math//topnum为数字计数器
